...
async fn main() {
                        ...
                        (
                                ...
                                Command::new("./multi_machine")
                                        .args([
                                                ...
                                                &config,
                                                "--pk_path",
                                                &NodeOpt::from_args().pk_path.to_string(),
                                        ])
                                        .stdout(Stdio::piped())
                                        .spawn()
                                        .expect(&format!(
                                        "Failed to start the multi_machine demo for node {}",
                                        id
                                )),
                        )
                })
        ...
        for (id, mut p) in processes {
                ...
                process.expect(&format!(
                        "Failed to run the multi_machine demo for node {}",
                        id
                ));
                ...
                loop {
                        stdout.read_line(&mut line)
                                .expect(&format!("Failed to read stdout for node {}", id));
                                ...
                                .starts_with(&format!("- Round {} completed. Commitment:", num_txn))
                        {
                                ...
                                let comm = strs.last().expect(&format!(
                                        "Failed to parse commitment for node {}",
                                        id
                                ));
