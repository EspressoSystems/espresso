...
/// Multi-machine consensus validation example
///
/// Look for the web server address in the console output for help.
#[derive(Debug, Deserialize, Serialize, StructOpt)]
pub struct Args {
    /// Configuration path. Defaults to
    ///    ~/tri/multi_machine/multi_machine.toml
...
pub fn write_config_file(config_file: &ConfigFile, path: &PathBuf) {
    ...
    create_dir_all(&dir).expect(&format!("Failed to create directory {:?}", &dir));
