...
pub enum EspressoError {
    ...
    Validation {
        source: espresso_core::state::ValidationError,
    },
...
impl From<espresso_core::state::ValidationError> for EspressoError {
    fn from(source: espresso_core::state::ValidationError) -> Self {
...
pub trait FromError: Sized {
    ...
    fn from_validation_error(source: espresso_core::state::ValidationError) -> Self {
        Self::catch_all(source.to_string())
    }
...
impl From<CommittedBlock> for ElaboratedBlock {
    fn from(b: CommittedBlock) -> Self {
        let (txs, proofs, memos, signatures) = b
            ...
            .multiunzip();
        Self {
            block: Block{txns: txs, fee_blind: &Self.block.fee_blind, proposer_pub_key: &Self.block.proposer_pub_key},
            proofs,
            memos,
            signatures,
        }
...
impl From<phaselock::types::PhaseLockError> for Error {
    fn from(source: phaselock::types::PhaseLockError) -> Self {
